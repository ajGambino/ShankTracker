...
const ROUND_SCHEDULE = [
  { order: 1, courseKey: 'indianBayou',      date: '2026-05-13', teeTime: '10:00' },
  { order: 2, courseKey: 'kellyPlantation',  date: '2026-05-14', teeTime: '09:06' },
...
const COURSE_JSON_PATHS = {
  indianBayou:     path.join(__dirname, '../notes/indian_bayou.json'),
...
function getTeeData(courseJson, gender, index) {
  ...
  if (index >= tees.length) throw new Error(`Tee index ${index} out of range (${tees.length} tees available)`);
  return tees[index];
}
...
async function main() {
  ...
  for (const entry of ROUND_SCHEDULE) {
    ...
    if (!jsonPath) throw new Error(`No JSON path configured for courseKey: ${entry.courseKey}`);
  ...
  for (const docSnap of roundsSnap.docs) {
    ...
    if (data.order != null) roundsByOrder[data.order] = { id: docSnap.id, data };
  }

  for (const entry of ROUND_SCHEDULE) {
    const round = roundsByOrder[entry.order];
    if (!round) {
      console.log(`  Round order=${entry.order}: NOT FOUND in Firestore — skipping`);
      continue;
    }
